...
const output = resolve(root, 'staging/media/krea2-canonical-sample/albina-upper-body-neutral-composite-six-lora-v1.png');
const receiptPath = output.replace(/\.png$/u, '.json');
const sourceDimensions = { width: 588, height: 1766, color: 0xf1f2f0 };
...
const systemPrompt = 'The original canonical crop composited over a neutral background is the sole authority for identity and authored design. Preserve the adult face, asymmetrical eyes, structured fringe, cable hair construction, formal black-and-white tailoring, and mechanical body language exactly. Do not substitute generic anime features.';
...
const workflow = buildKrea2ImageEditWorkflow(baseline, {
  prompt, negativePrompt, systemPrompt, seed: 2026081205,
  filenamePrefix: 'albina_upper_body_neutral_composite_six_lora_v1', aspectRatio: '2:3 (Portrait Photo)', megapixels: 1,
  subjectImage: sourceUpload.filename, subjectCompositeBackground: sourceDimensions, subjectCrop, subjectReferenceBoost: 4,
...
if (result.images.length !== 1) throw new Error(`Albina neutral composite probe expected one image, received ${result.images.length}`);
const saved = await downloadKrea2Image(result.images[0], output, { comfyUrl });
const receipt = {
  schemaVersion: 1, purpose: 'albina-upper-body-neutral-alpha-composite-probe-six-lora', status: 'awaiting-human-visual-review', createdAt: new Date().toISOString(),
  baseline: { workflowSha256: evidence.workflow.sha256, topologySha256: evidence.runtime.topologySha256 },
  references: { canonicalSource: { path: source, sha256: sha256(await readFile(source)), input: sourceUpload.filename, uploadedSha256: sourceUpload.sha256, role: 'sole canonical identity and authored design authority', crop: subjectCrop, preprocessing: { method: 'original alpha composited over neutral RGB', background: sourceDimensions } } },
  prompt, negativePrompt, systemPrompt, styleLoraChain,
  workflow: { sha256: sha256(JSON.stringify(workflow)), topologySha256: sha256(JSON.stringify(workflowTopology(workflow))), dualReferenceGrounding: false, preprocessing: 'LoadImageMask(alpha) + EmptyImage(neutral) + ImageCompositeMasked before crop, VAEEncode, Krea2EditGroundedEncode, and Krea2EditModelPatch' },
  execution: { comfyUrl, promptId: queued.promptId, seed: 2026081205, experiment: 'transparent-source neutral-composite encoding probe; not a production asset' }, output: saved,
