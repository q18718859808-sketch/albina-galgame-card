...
`v2.0.0-rc.4` is an immutable release-candidate tag. It may be published while `completeEdition` is false because the card and status file identify it as an RC and disclose its remaining limits. Never move or overwrite a published tag.
